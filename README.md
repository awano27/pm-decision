...
| 対象 | 正しい行動 | 人の確認へ | 安全側の代替行動 | 誤った行動 | 重大な取りこぼし |
|---|---|---|---|---|---|
| 調整用 99 件 | 62 | 25 | 12 | 0 | 0 |
| ホールドアウト 36 件（調整に未使用） | 29 | 6 | 0 | 1 | 0 |

誤った行動はほぼ出ず、迷うものは人に回す。1 件の判断は CPU で中央値 3.6 秒。実データで `eval/run_eval.py tune` による再調整を推奨。
